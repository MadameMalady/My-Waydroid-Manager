...
# Notes: This is a cli program to help Pinephone Pro owners running Manjaro (tested with Phosh) install and manage Waydroid on their phone. It's possible it's also compatible with OG pinephones on manjaro, i've not tested it.
...
function ACTIONS {
   ...
   Q: An application or game says it 
      won't run without Play Services!
   A: It's most likely lying to you;
      Tap anywhere off of the pop-up
      and it should continue to run in 
      most cases.
   
   Q: How do I type options 10 or 11 in 
      the extra-options page?
   A: hit the alt key on squeekboard, 
      or hold alt if using a keyboard,
      than type the number and hit enter.
        
   Q: What's the proper way to use 
      this program?
   A: Run each option one at a time:
   
      1: Install dependencies
      2: Install waydroid with either
      the second or third options
      (or go to 5, and install from
      pacman and sourceforge.)
      3: Toggle 'Software-based
      rendering' on
      4: restart the container to be 
      sure.
      5: You can toggle 'GPU-based
      rendering' on after if you'd like.
   ...
   A: If waydroid trys to show the full ui, 
      ...
      It will not render the 
      window correctly. Restart waydroid. 
      (option in extra options.)
   ...
   A: Open an Issue on github, 
    ...
    if [[ ${choices[1]} ]]; then
        ...
        notify-send "MWM" "This will install python-pip,
...
or \e[31mcrtl + c\e[0m to Cancel."
    ...
    if [[ ${choices[2]} ]]; then
        ...
        notify-send "MWM" "Installing 'waydroid' and 'waydroid-image-gapps'
packages from Aur. "
        echo -e "Press '\e[92m enter \e[0m' to continue
or \e[31mcrtl + c\e[0m to Cancel."
        read -p ""
        ...
        notify-send "MWM" "Remember to give the container 45 seconds to start"
        notify-send "MWM" "Remember to close the on-screen keyboard"
...
options[2]=" || Download Waydroid only ||
        || and Install the vanilla||
        || version of lineageos   ||
        || from Sourceforge       ||
        ||------------------------||"
options[3]=" || Download Waydroid only ||
        || And Install the gapps  ||
...
function ACTIONS {
	...
	#!/bin/bash
        notify-send "MWM" "Remember to give the container 45 seconds to start"
	notify-send "MWM" "Close the on-screen keyboard until Waydroid starts."
	waydroid session stop
        ...
        read -p "Press 'enter' to continue"
        notify-send "MWM" "Video Acceleration Disabled
        ( see ' Help! ' for more information on this
        option )"
        ./MWM.sh
    fi
    if [[ ${choices[1]} ]]; then
        ...
        notify-send "MWM" "Close the on-screen keyboard until Waydroid starts."
        waydroid session stop
        ( sudo echo "ro.hardware.gralloc=gbm" ; sudo echo "ro.hardware.egl=mesa" ; sudo echo "ro.opengles.version=196609" ) >>/var/lib/waydroid/waydroid_base.prop
        grep sys.use_memfd=true /var/lib/waydroid/waydroid_base.prop || echo sys.use_memfd=true | sudo tee -a /var/lib/waydroid/waydroid_base.prop
        sudo systemctl enable waydroid-container.service
        sudo systemctl restart waydroid-container.service
        waydroid session start &
        ...
        echo -e "Press '\e[92m enter \e[0m' to continue"
        read -p ""
        notify-send "MWM" "Video Acceleration Enabled
        ( see ' Help! ' for more information on this 
        option )"
        ./MWM.sh
    fi
    if [[ ${choices[2]} ]]; then
        # install lineageos vanilla from sourceforge
        notify-send "MWM" "This will download a vanilla 
vender and system image from sourceforge
and install it or you.
(source last updated 6/25/22)"
        echo -e "Press '\e[92m enter \e[0m' to continue 
or \e[31mcrtl + c\e[0m to Cancel."
        read -p ""
        sudo pacman -S waydroid
        sudo pacman -Syu
        sudo rm -rf /home/$me/lineage-vanilla-image
        cd /
        cd var
        cd lib
        cd waydroid
        ...
        mkdir images
        cd ~
        mkdir -p /home/$me/lineage-vanilla-image
        cd /home/$me/lineage-vanilla-image
        # system image first
        notify-send "MWM" " LineageOS ( Vanilla ) system.zip download
        from Sourceforge.net has started"
        wget -O  lineage-17.1-20220514-VANILLA-waydroid_arm64-system.zip  https://sourceforge.net/projects/waydroid/files/images/system/lineage/waydroid_arm64/lineage-17.1-20220514-VANILLA-waydroid_arm64-system.zip
        # next the vendor image
        notify-send "MWM " "LineageOS ( Vanilla ) vendor.zip downlaod
        from Sourceforge.net has started"
        wget -O  lineage-17.1-20220514-MAINLINE-waydroid_arm64-vendor.zip  https://sourceforge.net/projects/waydroid/files/images/vendor/waydroid_arm64/lineage-17.1-20220514-MAINLINE-waydroid_arm64-vendor.zip
        notify-send "MWM" "Image downlaods completed, extracting..."
        unzip lineage-17.1-20220514-MAINLINE-waydroid_arm64-vendor.zip
        unzip lineage-17.1-20220514-VANILLA-waydroid_arm64-system.zip
        notify-send "MWM" "Extraction completed, copying..."
        sudo find /home/$me/lineage-vanilla-image \( -name "*.img" -o -name "*.IMG" \) -exec cp -r {} /var/lib/waydroid/images/ \;
        grep sys.use_memfd=true /var/lib/waydroid/waydroid_base.prop || echo sys.use_memfd=true | sudo tee -a /var/lib/waydroid/waydroid_base.prop
        notify-send "MWM" "Remember to give the container 45 seconds to start"
        notify-send "MWM" "Remember to close the on-screen keyboard"
        sudo waydroid init
    ...
    if [[ ${choices[3]} ]]; then
        # Install gapps system.img and vender.img from sourceforge (source last updated 6/25/22)
        notify-send "MWM" "This will download a gapps 
vender and system image from Sourceforge.net
and install it or you.
(source last updated 6/25/22)"
        echo -e "Press '\e[92m enter \e[0m' to continue 
or \e[31mcrtl + c\e[0m to Cancel."
        ...
        mkdir images
        cd ~
        mkdir -p /home/$me/lineage-gapps-image
        cd /home/$me/lineage-gapps-image
        # system image first
        notify-send "MWM" " LineageOS ( Gapps ) system.zip download
        from Sourceforge.net has started" 
        wget -O  lineage-17.1-20220514-GAPPS-waydroid_arm64-system.zip  https://sourceforge.net/projects/waydroid/files/images/system/lineage/waydroid_arm64/lineage-17.1-20220514-GAPPS-waydroid_arm64-system.zip
        # than the vendor image
        notify-send "MWM " "LineageOS ( Vanilla ) vendor.zip downlaod
        from Sourceforge.net has started"
        wget -O  lineage-17.1-20220514-MAINLINE-waydroid_arm64-vendor.zip  https://sourceforge.net/projects/waydroid/files/images/vendor/waydroid_arm64/lineage-17.1-20220514-MAINLINE-waydroid_arm64-vendor.zip
        notify-send "MWM" "Image downlaods completed, extracting..."
        unzip lineage-17.1-20220514-GAPPS-waydroid_arm64-system.zip
        unzip lineage-17.1-20220514-MAINLINE-waydroid_arm64-vendor.zip
...
Hit enter to continue."
        read -p ""
        sudo mkdir -p /home/$me/MWM || true
        sudo mkdir -p /home/$me/MWM/Apk_Files || true
...
or \e[31mcrtl + c\e[0m to Cancel."
    ...
    if [[ ${choices[7]} ]]; then
        # Remove Waydroid
        notify-send "MWM" "This will Uninstall waydroid and 
waydroid-image from Aur or Manjaro Repositories; All images in
/var/lib/waydroid/images will also be deleted."
        notify-send "MWM" "Your installed applications will
        be left untouched, but unlaunchable until Waydroid
        and images are restored."
        echo -e "Press '\e[92m Enter \e[0m' to continue 
or \e[31mcrtl + c\e[0m to Cancel."
        read -p ""
        cd /
    ...
    if [[ ${choices[8]} ]]; then
        ...
        notify-send "MWM" "Waydroid session stopped"
        sudo waydroid container stop
        notify-send "MWM" "Waydroid container stopped"
        echo -e "Press '\e[92m Enter \e[0m' to continue."
        read -p ""
    fi
    if [[ ${choices[9]} ]]; then
...
ACTIONS
    # Developer note: I don't remember when I put these choices options here but they definitely need to be removed at some point as they're duplicates of other entries.
    fi
    if [[ $choices[5]} ]]; then
        # Credits
        echo "Credits: Cara Oswin, Waydroid Team"
        read -p "Press 'enter' to continue"
        notify-send "MWM" "Thanks for using Manjaro Waydroid Manager 
  -Cara Oswin"
        ./MWM.sh
    fi
    if [[ ${choices[6]} ]]; then
...
#Menu loop
while MENU && read -e -p "Please make a choice, for help, see option 1... " -n1 SELECTION && [[ -n "$SELECTION" ]]; do
    clear
    if [[ "$SELECTION" == *[[:digit:]]* && $SELECTION -ge 1 && $SELECTION -le ${#options[@]} ]]; then
        (( SELECTION-- ))
        if [[ "${choices[SELECTION]}" == "+" ]]; then
